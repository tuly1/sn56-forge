"""Production epoch cap for the evidenced Granite4.1-3B instruct LoRA route.
...
EXACT_MODEL_ID = "ibm-granite/granite-4.1-3b"
EXACT_BASE_MODEL = "/cache/models/ibm-granite--granite-4.1-3b"
EXACT_SOURCE_RANK = 32
EXACT_SOURCE_ALPHA = 64
EXACT_SOURCE_DROPOUT = 0.05
EXACT_TARGET_MODULES = tuple(
    sorted({"q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"})
)
...
def eligible_granite41_production_epoch_cap(
    ...
    """Recognize only the measured Granite4.1-3B production LoRA route."""
    if (
        strategy != "lora"
        or n_gpus != 1
...
def cap_granite41_production_epochs(
    spec: TaskSpec,
    model: Any,
    *,
    strategy: str,
    ...
    if not eligible_granite41_production_epoch_cap(
    ...
    try:
        ...
        raise ValueError("Granite4.1 production cap requires finite positive native epochs") from exc
    if not math.isfinite(observed) or observed <= 0.0:
        raise ValueError("Granite4.1 production cap requires finite positive native epochs")
