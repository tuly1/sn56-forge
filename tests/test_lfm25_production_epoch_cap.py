...
from forge.tuning import lfm25_epoch_cap as policy
...
def test_exact_route_caps_only_native_epochs_above_one() -> None:
    spec = _spec()
    model = _Model()
    assert policy.cap_lfm25_production_epochs(
...
@pytest.mark.parametrize(
    "spec",
    [
        _spec(model="b789d5139563461c"),
        _spec(
            model="B789D5139563461C",
            baseline_stats_path="/cache/baseline_stats_task.json",
        ),
        _spec(
            model="b789d5139563461c",
            baseline_stats_path="/cache/baseline_stats_task.json",
            cached_model_dir="/cache/models/LiquidAI--LFM2.5-2.6B",
        ),
    ],
)
...
def test_exact_route_rejects_invalid_native_epochs(native: float) -> None:
    with pytest.raises(ValueError, match="finite positive"):
        policy.cap_lfm25_production_epochs(
...
def test_cap_is_applied_before_training_arguments_and_keeps_native_cadence() -> None:
    ...
    cap = source.index("cap_lfm25_production_epochs(", assign)
    ...
    applied = policy.cap_lfm25_production_epochs(
...
def test_patch_does_not_add_experiment_capture_or_seed_controls() -> None:
    source = (
        Path(__file__).parents[1] / "forge" / "tuning" / "lfm25_epoch_cap.py"
    ).read_text(encoding="utf-8")
    for forbidden in (
        "FORGE_",
        "set_seed",
